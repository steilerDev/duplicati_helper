...
class Server(object):
    ...
    def request(self, command_string):
        ...
        logging.debug('Server: Telnet request: %s', command_string)
        while True:
        ...
        logging.debug('Server: Telnet response: %s', response[:-1])
        return response
    ...
    def poll(self):
        ...
        if select.select([self.tn], [], [], 0) == ([self.tn], [], []):
            response = urllib.unquote(self.tn.read_until(b"\n").decode())
            logging.debug('Server: Telnet Poll: %s', response[:-1])
            # TODO Keep track of which screen is displayed
            return response
        else:
            return None
    ...
    def add_screen(self, screen):     
        ...
        if screen.ref not in self.screens:   
            self.screens[screen.ref] = screen
            return self.screens[screen.ref]
